  ...
  return (
    ...
    >
      ...
      {currentTrack?.ringtone && (
        <View style={styles.ringtoneButtonContainer}>
          <TouchableOpacity 
            style={styles.ringtoneActionButton}
            onPress={() => setShowRingtoneModal(true)}
            data-testid="ringtone-button"
            accessibilityLabel="Set as Ringtone"
          >
            <Ionicons name="musical-note" size={18} color="#FFFFFF" />
            <Text style={styles.ringtoneActionButtonText}>Set as Ringtone</Text>
          </TouchableOpacity>
        </View>
      )}
          ...
          <View style={styles.ringtoneModalContent}>
            ...
            {currentTrack?.ringtone && (
              <TouchableOpacity
                ...
                data-testid="download-ringtone-btn"
                accessibilityLabel="Download ringtone"
                onPress={async () => {
                  ...
                  try {
                    ...
                    if (fileUri) {
                      ...
                      Alert.alert(
                        'Downloaded!',
                        isWeb 
                          ? 'Ringtone download started. Check your Downloads folder.'
                          : 'Ringtone saved successfully. You can now share it.',
                        [{ text: 'OK' }]
                      );
                    } else {
                      Alert.alert('Error', 'Failed to download ringtone. Please check your internet connection and try again.');
                    }
                  } catch (error: any) {
                    console.error('Download error:', error);
                    Alert.alert('Error', `Failed to download ringtone: ${error?.message || 'Unknown error'}`);
                  } finally {
            ...
            {currentTrack?.ringtone && (
              <TouchableOpacity
                ...
                data-testid="share-ringtone-btn"
                accessibilityLabel="Share ringtone"
                onPress={async () => {
                  if (!currentTrack?.ringtone) return;
                  ...
                  try {
                    ...
                    Alert.alert('Error', `Failed to share ringtone: ${error?.message || 'Unknown error'}`);
                  } finally {
                    setIsProcessingRingtone(false);
                    setRingtoneProgress(0);
                  }
