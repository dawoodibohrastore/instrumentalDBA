...
// Download ringtone for web - opens URL directly for download (CORS-safe)
export const downloadRingtoneWeb = async (
  ringtoneUrl: string,
  trackTitle: string
): Promise<boolean> => {
  try {
    ...
    // Method 1: Try using anchor tag with download attribute
    // This works for same-origin or CORS-enabled URLs
    const a = document.createElement('a');
    a.href = ringtoneUrl;
    a.download = `${sanitizedTitle}_ringtone.mp3`;
    a.target = '_blank';
    ...
    try {
      window.open(ringtoneUrl, '_blank');
      return true;
    } catch (e) {
...
// Share ringtone for web - copies URL to clipboard or opens share dialog
export const shareRingtoneWeb = async (
  ringtoneUrl: string,
  trackTitle: string
): Promise<boolean> => {
  try {
    ...
    if (navigator.share) {
      await navigator.share({
        title: `${trackTitle} - Ringtone`,
        text: `Check out this ringtone: ${trackTitle}`,
        url: ringtoneUrl,
      });
      return true;
    }
    ...
    if (navigator.clipboard) {
      await navigator.clipboard.writeText(ringtoneUrl);
      Alert.alert('Copied!', 'Ringtone URL copied to clipboard');
      return true;
    }
    
    // Final fallback: show alert with URL
    Alert.alert('Share Ringtone', `Ringtone URL: ${ringtoneUrl}`);
    return true;
  } catch (error) {
...
// Download ringtone directly from URL (Native - Mobile)
export const downloadRingtoneNative = async (
  ringtoneUrl: string,
  trackId: string,
  trackTitle: string,
  ...
  try {
    ...
    console.error('Error downloading ringtone (native):', error);
    return null;
  }
};

// Main download function - handles both web and native
export const downloadRingtone = async (
  ...
  if (isWebPlatform()) {
    onProgress?.(0.5);
    const success = await downloadRingtoneWeb(ringtoneUrl, trackTitle);
    onProgress?.(1);
    return success ? ringtoneUrl : null; // Return URL as "file path" for web
  }
  
  return downloadRingtoneNative(ringtoneUrl, trackId, trackTitle, onProgress);
};

// Share ringtone file (Native)
export const shareRingtoneNative = async (
  fileUri: string,
  title: string
...
// Main share function - handles both web and native
export const shareRingtone = async (
  fileUriOrUrl: string,
  title: string
): Promise<{ success: boolean; message: string }> => {
  if (isWebPlatform()) {
    const success = await shareRingtoneWeb(fileUriOrUrl, title);
    return {
      success,
      message: success ? 'Shared successfully!' : 'Failed to share.',
    };
  }
  
  return shareRingtoneNative(fileUriOrUrl, title);
};
...
export const downloadAndShareRingtone = async (
  ...
  try {
    if (isWebPlatform()) {
      // For web, just share the URL directly
      onProgress?.(0.5);
      const success = await shareRingtoneWeb(ringtoneUrl, trackTitle);
      onProgress?.(1);
      return {
        success,
        message: success ? 'Shared successfully!' : 'Failed to share.',
    ...
    // For native, download then share
    const fileUri = await downloadRingtoneNative(ringtoneUrl, trackId, trackTitle, onProgress);
    ...
    const shareResult = await shareRingtoneNative(fileUri, trackTitle);
